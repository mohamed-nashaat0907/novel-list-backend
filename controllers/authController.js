  ...
  try {
    ...
    if (!currentUser.active) {
      return res.status(403).json({
        status: "fail",
        message:
          "Your account has been deactivated. Please contact the admins.",
      });
    }
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    if (
      ...
      now - user.lastEmailSentAt.getTime() < ONE_MINUTE
    ) {
      return res.status(429).json({
        status: "fail",
        message:
          "You must wait 1 minute before requesting another password reset email.",
      });
    }
