...
const createOrder = async (req, res) => {
    try {
        ...
        // If PayPal credentials are missing or clearly placeholder values, return a mocked approval URL
        const isMockPayPal = !process.env.PAYPAL_CLIENT_ID || process.env.PAYPAL_CLIENT_ID.toLowerCase().includes('fake') || !process.env.PAYPAL_CLIENT_SECRET;
        if (isMockPayPal) {
            const mockApprovalUrl = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/checkout/success-popup?mock=true&trx=${trxRef}`;

            // For mock flow, mark order as processed/paid immediately so frontend "success" matches backend state
            const newOrder = new Order({
                ...
                orderNumber,
                paypalOrderId: `MOCK-${trxRef}`,
                transactionRef: trxRef,
                paypalCaptureId: `MOCK-CAP-${trxRef}`,
                paidAt: new Date()
            });
            ...
            return res.status(201).json({
                ...
                data: {
                    ...
                    paypalOrderId: savedOrder.paypalOrderId,
