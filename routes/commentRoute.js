...
// ✅ عرض الكومنتات المعتمدة فقط للمستخدم النهائي
router.get("/:id", async (req, res) => {
  ...
  try {
    const comments = await commentController.listComments(id, true); // فقط الكومنتات المعتمدة
...
// ✅ عرض جميع كومنتات المستخدم مع حالة كل كومنت
...
router.get("/check/:bookId", Authenticate, async (req, res) => {
  ...
  try {
    ...
    const isBought =
      Array.isArray(order) && order.some((o) => o.status === "processing");
    ...
    res.status(200).json({ isBought, isReviewed });
  } catch (err) {
...
// === New Admin Endpoints ===
router.get("/admin/comments", AuthenticateAdmin, async (req, res) => {
  try {
    const { status, productId, userId, page = 1, limit = 10 } = req.query;
    const result = await commentController.getAllCommentsAdmin({
      status,
      bookId: productId,
      userId,
      page,
...
router.patch("/admin/comments/:id", AuthenticateAdmin, async (req, res) => {
  const { id } = req.params;
  const { status } = req.body;

  try {
    const updatedComment = await commentController.updateCommentStatus(
      id,
      status
    );
    res.status(200).json({
